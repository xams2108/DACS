...
export function WalletImage(props) {
    ...
    if (image?.authProvider === "email") {
        return _jsx(EmailIcon, { size: props.size, color: theme.colors.accentText });
    }
    if (image?.authProvider === "phone") {
        return _jsx(PhoneIcon, { size: props.size, color: theme.colors.accentText });
    }
    if (image?.authProvider === "passkey") {
        return (_jsx(FingerPrintIcon, { size: props.size, color: theme.colors.accentText }));
    }
    if (image?.authProvider === "wallet") {
        return (_jsx(OutlineWalletIcon, { size: props.size, color: theme.colors.accentText }));
    }
    if (image?.authProvider === "guest") {
        return _jsx(GuestIcon, { size: props.size, color: theme.colors.accentText });
    }
    if (image?.uri) {
        return (_jsx(Img, { src: image.uri, width: props.size, height: props.size, loading: "eager", client: props.client, style: {
