...
function EcosystemWalletConnectUI(props) {
    ...
    if (state?.walletLogin) {
        return (_jsx(WalletAuth, { meta: props.meta, chain: props.chain, inAppLocale: locale, walletConnect: props.walletConnect, wallet: props.wallet, client: props.client, size: props.size, done: done, onBack: goBackToMain || (() => setSelectionData({})), locale: props.connectLocale, isLinking: state.walletLogin.linking }));
    }
    if (state?.guestLogin) {
